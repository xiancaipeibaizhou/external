"""Adapted ShuffleFAC Stage-1 training and evaluation entry point.

This script adds reproducible dataset discovery, strict recording-level
splits, feature caching, and artifact saving around the upstream model.
"""
...
def discover_segments(
    ...
    # Expected layout: one directory per class, with wavs either directly
    # under the class folder or inside recording subdirectories.
...
def split_recording_level(entries, seed: int, train_ratio: float, val_ratio: float, test_ratio: float):
    # Split by recording_id first so clips from the same recording never leak.
    rng = random.Random(seed)
...
def build_feature_cache(
    ...
    if cache_path.exists():
        # Cache keys include split metadata and feature config, so stale caches are ignored.
...
def aggregate_recording_scores(logits, probs, preds, strategy):
    """Aggregate clip-level predictions into one recording-level decision."""
