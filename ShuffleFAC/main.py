"""Original upstream ShuffleFAC training entry point.

The workspace experiments usually use run_deepship.py instead because it
adds reproducible splits, feature caches, and complete artifact saving.
"""
...
def train(student, train_loader, optimizer, criterion, device):
    ...
    for batch_x, batch_y in tqdm(train_loader, total=len(train_loader), desc='Train', leave=False, dynamic_ncols=True):
        ...
        # Per-batch cross entropy over log-mel segments.
...
def evaluate(student, val_loader, criterion, device):
    ...
    # Macro-F1 is the main validation signal for class imbalance.
