...
class NoiseGraphAttentionHead(nn.Module):
    ...
    def _build_graph(self, z_noise):
        ...
        if self.edge_mode == "threshold_similarity":
            # Threshold mode keeps temporal neighbors and adds all noise-space
            # neighbors whose cosine similarity clears sim_threshold.
        ...
        # Record the true degree before padding repeated indices for gather().
...
class SignalNoiseDecoupledModel(nn.Module):
    ...
    def encode_nodes(self, clips):
        ...
        # Downstream ablations train only the head; gradients must not reach ShuffleFAC.
        return emb.detach()
    ...
    def topk_masked_softmax(self, signal_scores):
        ...
        if hasattr(self, "signal_top_k") and self.signal_top_k > 0 and s > 1:
            # signal_top_k=0 is the backward-compatible full-attention path.
            apply_topk = True
            if (
                # Warmup keeps the original full soft attention for early epochs.
                self.training
        ...
        if apply_topk:
            ...
            # Use dtype min instead of -inf so mixed precision stays well-defined.
