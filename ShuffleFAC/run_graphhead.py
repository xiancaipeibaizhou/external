...
class RecordingBagCachedDataset(Dataset):
    ...
    def __init__(
        ...
        # Convert clip-level cache rows into one bag per recording.
    ...
    def _sample_indices(self, indices, eval_sample_id: int = 0, eval_samples: int = 1):
        ...
        if self.train:
            # Training uses random clips from the recording; evaluation is deterministic.
            if n >= s:
...
class FrozenShuffleFACEncoder(nn.Module):
    def __init__(self, checkpoint_path: Path, device: torch.device):
        ...
        # This wrapper exposes CNN embeddings while keeping classifier weights unused.
        self.encoder.eval()
...
class AttentionAggregationHead(nn.Module):
    ...
    def pool(self, nodes):
        # Attention pooling turns a variable recording bag into one recording vector.
...
class GraphAggregationHead(AttentionAggregationHead):
    ...
    def _build_graph(self, nodes):
        ...
        if self.edge_mode in ("similarity", "temporal_similarity"):
            # Similarity edges are top-k in frozen-encoder embedding space.
...
class GraphAwareAttentionHead(GraphAggregationHead):
    ...
    def forward(self, x):
        ...
        # Graph context influences attention scores, but pooling still uses original nodes.
