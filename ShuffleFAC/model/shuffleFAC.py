"""ShuffleFAC lightweight CNN backbone.

The model keeps the original upstream naming. In this workspace it is used both
as a standalone classifier and as a frozen encoder for recording-level heads.
"""
...
class GLU(nn.Module):
    """Gated linear unit applied channel-wise on 2D acoustic feature maps."""
...
class ContextGating(nn.Module):
    """Learns a sigmoid channel gate and multiplies it with the input map."""
...
class SelfAttention(nn.Module):
    """Computes per-channel gates from the average frequency profile."""
...
class fac_conv(nn.Module):
    """Frequency-aware correction: inject learned frequency position with attention."""
...
class ChannelShuffle(nn.Module):
    """Shuffle grouped channels so later grouped convolutions can mix information."""
    ...
    def __init__(
        ...
        **transformer_kwargs
        ...
        def conv(i, normalization="batch", dropout=None, activ="relu"):
            ...
            if i == 0:
                # The first layer is a regular convolution after FAC injection.
                cnn.add_module(f"fac_conv{i}", fac_conv(f_bins=freq_bins[i])),
                cnn.add_module(f"conv{i}", 
                nn.Conv2d(nIn, nOut, kernel_size[i], stride[i], padding[i]),)
            else:
                # Later blocks use the lightweight ShuffleNet-style pattern:
                # grouped 1x1 -> depthwise spatial conv -> channel shuffle -> grouped 1x1.
                cnn.add_module(f"fac_conv{i}", fac_conv(f_bins=freq_bins[i],)),
...
class shuffleFAC(nn.Module):
        ...
        def forward(self, x):
            # Training/eval loaders provide [B, C, F, T]; CNN expects [B, C, T, F].
            x = x.transpose(2,3)
            x = self.cnn(x)
            x = x.view(x.size(0), -1)
            x = self.fc(x)
